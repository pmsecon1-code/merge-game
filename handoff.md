...
│   ├── spawners/       # 생성기 이미지 8종
│   ├── badges/         # 배지 바 아이콘 6종
│   ├── story/          # 보스 이미지 7종 WebP + scenes/ 24장 WebP
│   │   └── scenes/     # 스토리 씬 이미지 24장 WebP (EP.1~7)
│   ├── cats/           # 고양이 동물 이미지 11종
│   ├── dogs/           # 강아지 동물 이미지 11종
│   ├── birds/          # 새 동물 이미지 7종
│   ├── fish/           # 물고기 동물 이미지 7종
...
**보스 이미지** (`images/story/`, 7종, WebP)
| 파일명 | EP |
|--------|-----|
| `boss_shadow.webp` | EP.1 |
| `boss_scout.webp` | EP.2 |
| `boss_trapper.webp` | EP.3 |
| `boss_archer.webp` | EP.4 |
| `boss_pirate.webp` | EP.5 |
| `boss_guard.webp` | EP.6 |
| `boss_king.webp` | EP.7 |

**씬 이미지** (`images/story/scenes/`, 24종, WebP)
`ep1_intro_1.webp`, `ep1_intro_2.webp`, `ep1_outro_1.webp`, `ep2_intro_1.webp`, `ep2_intro_2.webp`, `ep2_outro_1.webp`, `ep3_intro_1.webp`, `ep3_intro_2.webp`, `ep3_outro_1.webp`, `ep4_intro_1.webp`, `ep4_intro_2.webp`, `ep4_outro_1.webp`, `ep5_intro_1.webp`, `ep5_intro_2.webp`, `ep5_outro_1.webp`, `ep6_intro_1.webp`, `ep6_intro_2.webp`, `ep6_intro_3.webp`, `ep6_outro_1.webp`, `ep7_intro_1.webp`, `ep7_intro_2.webp`, `ep7_intro_3.webp`, `ep7_outro_1.webp`, `ep7_outro_2.webp`
...
- 💥 **합성 타격감 강화**
  - `merge-punch` CSS 애니메이션 (0.3s scale 1→1.2→1) 합성 시 적용
  - `spawnParticles(cell, level)` 레벨별 차등: 수량 `min(6+level*2, 16)`, 색상 저(핑크+보라) 중(핑크+금) 고(금+빨강), 퍼짐 `30+level*5`
  - `screenShake(intensity)` 합성 시 보드 흔들림 (최대 4px, 0.15s)
  - "UP!" → "Lv.N!" 빨간색 floatText로 변경
  - `showFloatText` 중앙정렬 + text-shadow 가독성 개선
  - 합성 시 5단계 이펙트 시퀀스: merge-punch → particles → itemEffect → screenShake → floatText(50ms delay)
- 🎁 **보상 연출 강화**
  - `flyRewardToStatusBar(fromEl, type)` 아이콘 3~5개가 상단바로 날아가는 애니메이션 (0.5s, 60ms 딜레이)
  - 도착 시 상단바 `.status-bump` 범프 효과 (0.3s scale 1→1.25→1)
  - `spawnLevelupConfetti()` 레벨업 오버레이에 20개 6색 파티클 낙하 (1.2s)
  - 퀘스트 완료 시 체크마크 오버레이 (`quest-complete-check`, pop 등장)
...
- 🖼️ **스토리 이미지 PNG → WebP 전환** (보스 8종 + 씬 24장, 용량 ~75% 절감)
  - `images/story/*.png` → `*.webp`, `images/story/scenes/*.png` → `*.webp`
  - `js/constants.js` STORY_IMAGES 참조 경로 .png → .webp
- ⚖️ **EXPLORE_MAP 타일 보상 ~2배 상향** (코인 싱크 유지하면서 체감 보상 개선)
  - 타일 코인 합계: ~2,230 → ~4,450🪙 (회수율 5%→12%, 여전히 강한 싱크)
  - 타일 다이아: ~27 → ~47💎
  - 타일 에너지: ~130 → ~248⚡
  - 타일 카드: ~41 → ~76🃏
- ⚖️ **EXPLORE_MILESTONES 대폭 상향** (총합 1,100🪙+60💎 → 3,700🪙+320💎)
  - 3개: 100🪙+5💎 → 200🪙+10💎
  - 5개: 200🪙+10💎 → 500🪙+30💎
  - 7개: 300🪙+15💎 → 1,000🪙+80💎
  - 10개: 500🪙+30💎 → 2,000🪙+200💎 (+공룡 생성기 유지)
- 수정 파일: js/constants.js, js/story.js, js/main.js, js/ui.js, index.html, firebase.json, package.json (7개)
- 수정 상수: `EXPLORE_MAP` (38개 보상 타일 min/max 상향), `EXPLORE_MILESTONES` (4단계 coins/diamonds 상향)
- 이미지 변경: PNG 32개 삭제 + WebP 32개 추가
