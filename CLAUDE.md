...
js/constants.js     # 상수/데이터/ICON (~541줄)
js/state.js         # 전역 변수 (~126줄)
js/auth.js          # 인증/세션/회원탈퇴 (~177줄)
js/save.js          # 저장/로드 (~575줄)
js/game.js          # 게임 로직 (~843줄)
js/systems.js       # 미션/주사위여행/상점 (~479줄)
js/album.js         # 앨범 시스템 (~248줄)
js/race.js          # 레이스 시스템 (~1073줄)
js/sound.js         # 사운드 (효과음+BGM) (~410줄)
js/tutorial.js      # 온보딩 튜토리얼 (~194줄)
js/ui.js            # UI/렌더링/배지바/설정 (~720줄)
js/main.js          # 초기화 (~299줄)
...
## 상세 컨텍스트
@handoff.md - 전체 아키텍처, 함수 목록, 밸런스, 변경 이력 (v4.25.0)
