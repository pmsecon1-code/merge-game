...
css/styles.css      # 전체 스타일 (~1821줄)
js/constants.js     # 상수/데이터/ICON (~621줄)
js/state.js         # 전역 변수 (~132줄)
js/auth.js          # 인증/세션/회원탈퇴 (~180줄)
js/save.js          # 저장/로드 (~714줄)
js/game.js          # 게임 로직 (~968줄)
js/systems.js       # 미션/주사위여행/상점 (~444줄)
js/album.js         # 앨범 시스템 (~243줄)
js/race.js          # 레이스 시스템 (~1069줄)
js/sound.js         # 사운드 (효과음+BGM) (~419줄)
js/story.js         # 스토리 미션 시스템 (~325줄)
js/tutorial.js      # 온보딩 튜토리얼 (~194줄)
js/ui.js            # UI/렌더링/배지바/설정 (~833줄)
js/main.js          # 초기화 (~315줄)
...
## 상세 컨텍스트
@handoff.md - 전체 아키텍처, 함수 목록, 밸런스, 변경 이력 (v4.31.0)
